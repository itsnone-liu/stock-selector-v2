# 批次5语义规范：复权收益接入与正负路径分析（v7.2：v7.1 冻结版 + staged next 边界补记）

> 版本链：v1(d4b43ac)→v2(2f9ceee)→v3(4457bea)→v4(b00c52b)→v5(444d487)
> →v6(19c40db)→v7(1692b13/2637fdf)→v7.1(fcac5ee/f6f3015，正式冻结)。
...
**勘误（2026-09-20，用户批准）——采集 universe 修正**：原采集清单
`5,594`（SHA `1b2ac933635a…`）因 `sz3` 前缀规则混入 354 只 `sz399xxx`
深市指数。修正为**纯股票 5,240 只**（sh6 主板+科创板 2,330｜sz00x 主板
1,499｜sz30x 创业板 1,411；排除 sz399/880/800 与 sh9 B 股），新冻结
SHA256 `9de70c4fb77628a6692e7f0aa8ac0e11dee6447b3711a9fb02d1f3c1bd49fff6`。
三源已完成部分零污染（指数按字典序排在尾部均未轮到），manifest 自动
迁移、进度零损失。详见源审计报告 erratum 节。
...
**边界扩展（v7.2，2026-09-21 补记，来源 retcalc_v1 复审）——staged next 视角 T1 未成交而后续批次成交**：next 视角各批独立判断次日可成交性（Stage4 v5 冻结实现），T1 次日失败不丢弃后续批次。此时：
- 该组合**进入** K2/K4 分母（存在实际持仓），未成交的 T1 批次按现金 0 计入；
- K2/K4 共同终点 = **第一笔实际成功成交批次后第 h 交易日**（close 视角 T1 恒成交故仍为 T1 后 h，与本表原文一致；next 视角起点右移）；
- K3 不受影响（终点恒为突破后第 h 交易日，仅分批现金语义）。
