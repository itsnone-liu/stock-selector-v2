...
1. **恢复生命周期非常快**（T6.2+T7.3 双源）：REDUCE→ADD 中位约 2 天；
   ...
   P(est≤R+3)=77.5%。第一次 re-expansion 的**及时性**本身有值
   （T7.3：A wret_R40=+2.08%，全部延迟/确认类为负）。
...
- **值得**：第一次、及时（恢复确认点 median=R+3 之前后附近，A 的行为模式），
  且系统在 5 个有效 observation 内会自动告诉你对错（假恢复快速显形）。
- **不值得**：为"确认"而等待到 R+5 之后（错过 82% 的恢复建立点，且风险不降）；
  以 bounce 强度为过滤条件（区分力不转化为可执行性）。
- **未回答**：第一次成功后的后续 re-risk 序列（§3.1）——这是 T7 留下的
  结构最清晰的新问题。
