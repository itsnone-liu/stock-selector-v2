...
全部组同向恶化（方向成立），但**幅度强分层**：不修复组的恶化约为修复组的 3–6 倍。
恶化程度本身是 cycle 结局的强预测子。
...
**预注册的"修复后重入"读法被拒绝（C207 NOT_SUPPORTED）**：ADD 发生在价格进一步回落
之后，五个状态量中四个更差、只有 turnover 降温。机制更接近"轻度恶化 → 进一步下跌 →
低位买回"，不是"等待指标确认修复"。
