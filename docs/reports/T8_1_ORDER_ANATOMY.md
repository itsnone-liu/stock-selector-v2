...
1. **"错的风险"不随次数上升**：False ADD 率 k1→k3+ 平坦（44.9%→43.2%）。
   路径真假恢复的先验概率不随 re-risk 次数系统性变差——"越加越错"不成立。
2. **"对的收益"在下降**：恢复捕获 −9.8pp；中位贡献 ≈ 减半（0.0134→
   0.0069）。分解显示递减主要来自 **ret 中位下降 + dd 加深**，而非 upside
   消失——**mfe 平坦（6.3%→6.1%）**：第 3+ 次的最好情况与第 1 次相当，
   但中位结果变差、回撤更深（尾部机会保留、中位质量下降）。
3. 该混合结构**是否被 outcome₁ 条件化解释**（RQ2 / Estimand B）是 T8.2
   的预注册问题——本报告不做任何条件分解。
