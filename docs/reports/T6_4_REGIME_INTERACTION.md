...
## 1. 对核心问题的直接回答（C402）：市场 regime 不区分同签名的真假恢复
...
**分层后各 regime 的假恢复率都在 46–57%，CI 全部跨零**。外部 PIT 市场态不提供同签名
真假恢复的区分条件（NOT_SUPPORTED）。
...
反向不显著（−2.5pp，p=0.215）。由于该对比在热签名子集内消失，validation 效应与
**构成效应**一致（强市里哪些 cycle 进入恢复池），而非 regime 对单个 cycle 增加区分信息
（C403 PARTIALLY）。
...
p_terminal_failure 同构（E3 全层最低）。**E mapping 的风险塑形结构不是市场态伪影**
（C404 SUPPORTED）。

## 4. C106 的 regime 放大（C405）

E2 的 terminal-failure 超额集中在弱市：M_WEAK 层 E2−E1 = +4.5pp（val）/ +3.7pp（conf），
M_STRONG 层 conf 仅 +1.2pp——E2 的"带伤到末日"形态被弱市放大（C405 PARTIALLY，
与 T6.3 F 分解互补：多路径张力 + 弱市放大）。
...
2. 真假恢复的区分条件**不在**外部市场态（C402）——若存在区分信息，更可能在个股级
   路径内部（交 T6.5 综合 T6.2/T6.3 的恶化幅度 + 签名方向）；
3. 弱市是循环结构的放大器（67% no_recovery；E2 张力弱市放大）——但 conf 段形态不同，
   regime 叙事必须分段。
